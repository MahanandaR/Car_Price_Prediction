...
async function predict() {
  ...
  if (!brand || !model || !fuel_type || !transmission || !owner_type || !year || !km_driven || !engine_capacity) {
    ...
    outputBox.textContent = "⚠️ Please fill all details before prediction.";
    return;
  }
   
  // 🚫 KM Validation
  if (km_driven <= 1000 || km_driven > 200000) {
  outputBox.style.color = "red";
  outputBox.textContent = "❌ Invalid KM Driven value. Please enter a value between 1,000 and 2,00,000 km.";
  return;
}

if (engine_capacity <= 700 || engine_capacity > 200000) {
  outputBox.style.color = "red";
  outputBox.textContent = "❌ Invalid Engine Capacity value. Please enter a value between 700 and 3,000 cc.";
  return;
}
  ...
  try {
    const res = await fetch("https://car-price-prediction-xayj.onrender.com/predict", {  // 🔹 Change this if your API runs elsewhere
      method: "POST",
  ...
  outputBox.innerHTML = `<strong>💰 Estimated Car Price Range:</strong> ${result.output}`;
}
    } else {
      outputBox.style.color = "red";
      outputBox.textContent = "❌ Failed to get prediction. Please try again later.";
    }
  } catch (error) {
